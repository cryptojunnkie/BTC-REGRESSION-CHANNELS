...
colors = [(0.6, 0.3, 0.8), (0.1, 0.5, 0.9), (0.4, 0.6, 0.2), (0.9, 0.7, 0.1),
          (0.7, 0.2, 0.5), (0.3, 0.8, 0.6), (0.8, 0.5, 0.1), (0.2, 0.4, 0.7)]  # RGB color palette for lines
...
# Plot lines below the polynomial regression curve
for i in range(4):
    ...
    # Annotate the lower bands with buy area labels
    buy_annotation_offset = mdates.date2num(x[-1]) + 10
    plt.annotate('    BUY AREA ' + str(i + 1), xy=(buy_annotation_offset, y_fit[-1] - distance_below),
                 xytext=(buy_annotation_offset + 12, y_fit[-1] - distance_below),
                 arrowprops=dict(facecolor=colors[i], arrowstyle='->'), fontsize=8, ha='left')

# Plot lines above the polynomial regression curve
for i in range(4):
    ...
    # Annotate the upper bands with take profit area labels
    profit_annotation_offset = mdates.date2num(x[-1]) + 10
    plt.annotate('    TAKE PROFIT AREA ' + str(i + 1), xy=(profit_annotation_offset, y_fit[-1] + distance_above),
                 xytext=(profit_annotation_offset + 12, y_fit[-1] + distance_above),
                 arrowprops=dict(facecolor=colors[i + 4], arrowstyle='->'), fontsize=8, ha='left')
...
if price_high > 0.01:
    ...
    ax.yaxis.set_major_locator(MultipleLocator(10000))  # Set y-axis price interval to the nearest 500
else:
    ax.yaxis.set_major_formatter('${x:,.10f}')  # Use all place values to the right of the decimal for price
